...
// TestCINSArmyIntegration fetches the real CINS Army list, parses it,
// and verifies all entries are valid IPv4 with category "scanner".
//
// Run:  go test -tags=integration -run '^TestCINSArmyIntegration$' ./features/providers/cinsarmy/
func TestCINSArmyIntegration(t *testing.T) {
	req, err := http.NewRequest(http.MethodGet, cinsURL, nil)
	...
	err = parseCINSData(data, collector, providerName, cinsURL, "intg-process-id")
	require.NoError(t, err)

	count := len(collector.entries)
	assert.Greater(t, count, 10000, "expected >10K IPs, got %d", count)
	t.Logf("Parsed %d entries", count)

	// Verify every entry
	for i, e := range collector.entries {
		assert.Equal(t, providerName, e.Source, "[%d] source mismatch", i)
		assert.Equal(t, "scanner", e.Category, "[%d] category must be scanner", i)
		assert.Equal(t, "intg-process-id", e.ProcessID, "[%d] process ID mismatch", i)
		assert.Equal(t, cinsURL, e.SourceURL, "[%d] source URL mismatch", i)
...
func TestCINSArmyIntegration_NoDuplicatesInSource(t *testing.T) {
	req, err := http.NewRequest(http.MethodGet, cinsURL, nil)
	require.NoError(t, err)
	req.Header.Set("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36")
